...
# Streaming ASR server (TCP / Unix socket)
server: src/nemo-server.cpp $(GGML_SRCS) $(GGML_STREAM_SRCS)
	$(CXX) $(CXXFLAGS) $^ $(LDFLAGS) -o nemotron-server

clean:
	rm -f test_ggml_weights test_ggml_compute precompute_encoder_ref transcribe test_streaming transcribe_stream test_python_ref test_preprocessor nemotron-server
